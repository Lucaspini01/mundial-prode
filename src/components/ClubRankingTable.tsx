...
export default function ClubRankingTable({
  entries,
  userClubId,
}: {
  ...
  if (entries.length === 0) {
    return (
      <p className="text-center text-slate-400 py-8">No hay datos de ranking todavía.</p>
    );
  }
      ...
      {podium.length > 0 && (
        <div className="flex items-end justify-center gap-4 mb-8 pb-6 border-b border-slate-100">
          {[podium[1], podium[0], podium[2]].filter(Boolean).map((entry) => {
            const isMyClub = entry.clubId === userClubId;
            ...
            return (
              ...
              >
                ...
                />
                <span className={`text-xs font-bold truncate max-w-[72px] text-center ${isMyClub ? "text-green-700" : "text-slate-700"}`}>
                  {entry.clubShortName}
                </span>
        ...
        <table className="w-full">
          <thead>
            <tr className="text-left text-[11px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-100">
              ...
              <th className="pb-3 flex-1">Nombre</th>
              <th className="pb-3 text-right">Hinchas</th>
              ...
              return (
                <tr
                  ...
                  className={`transition-colors ${
                    isMyClub ? "bg-green-50 hover:bg-green-100/70" : "hover:bg-slate-50"
                ...
                >
                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-2">
                      <span className={`text-sm ${isMyClub ? "text-green-700 font-bold" : "text-slate-700"}`}>
                        {entry.clubName}
                      </span>
                      {isMyClub && (
